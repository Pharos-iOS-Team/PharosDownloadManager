...
final class DownloadFileViewModel: ObservableObject, Identifiable {
  ...
  init(file: DownloadableFile) {
    ...
    } else if PharosDownloadManager.shared.shouldAutoResume(id: file.id) {
      print("🔄 Auto-resuming \(file.id) due to previous intent")
      PharosDownloadManager.shared.download(item: file)
    }
    
    PharosDownloadManager.shared.progressPublisher(for: file.id)
      .receive(on: DispatchQueue.main)
      .sink { [weak self] value in self?.progress = value }
      .store(in: &cancellables)
    
    PharosDownloadManager.shared.$downloadStates
  ...
  // MARK: Actions
  func download() { PharosDownloadManager.shared.download(item: file) }
  func pause() { PharosDownloadManager.shared.pause(item: file) }
  func resume() { PharosDownloadManager.shared.resume(item: file) }
  func cancel() { PharosDownloadManager.shared.cancel(id: file.id) }
  func delete() {
    PharosDownloadManager.shared.delete(for: file)
    state = .idle
  }
}
